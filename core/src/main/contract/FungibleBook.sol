...
contract FungibleBook {
    ...
    function write(address[] transactionAddress, uint256 amount, string[] memory stringValueList, int[] typeList) public returns (bool, uint[2]){
    ...
    function setRecordEntry(uint termNo, uint seqNo, address[] transactionAddress, uint256 amount, string[] stringValueList, int[] typeList) internal returns (Entry){
        Table table = tableFactory.openTable(tableName);
        ...
        if (stringValueList.length > 1) {
            recordEntry.set("subject", stringValueList[1]);
        }
    ...
    function genExtenalAddress(string _from, string _to, address operator) internal view returns (string[]){
        ...
        if (address(0) != stringToAddress(_from)) {
        ...
        if (address(0) != stringToAddress(_to)) {
