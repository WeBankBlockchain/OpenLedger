...
contract AuthTable {
    ...
    function grant(address ownerId, address resource, address allowId, string operation, bytes detail) public onlyManager returns (bool) {
    ...
    function check(address ownerId, address resource, address caller, string operation, bytes detail) public onlyManager returns (bool){
        if (ownerId == caller) {
            return true;
        }
        bytes32 ownerResource = keccak256(ownerId, resource);
    ...
    function revoke(address ownerId, address resource, address caller, string operation, bytes detail) public onlyManager returns (bool){
        bytes32 ownerResource = keccak256(ownerId, resource);
    ...
    function createAuthEntry(bytes32 ownerResource, address allowId, string operation, bytes detail) internal returns (int256){
        ...
        if (detail.length > 0) {
            authEntry.set("detail", keccak256(detail).bytes32ToString());
        }
    ...
    function deleteAuthEntry(bytes32 ownerResource, address allowId, string operation, bytes detail) internal returns (int256){
        require(bytes(operation).length > 0, "required operation can not be null");
        Table table = tableFactory.openTable(tableName);
        Condition condition = table.newCondition();
        condition.EQ("operation", operation);
        if (detail.length > 0) {
            condition.EQ("detail",keccak256(detail).bytes32ToString());
        }
    ...
    function selectEntryByCallerId(bytes32 ownerResource, address callerId, string operation, bytes detail) internal returns (int256){
        require(bytes(operation).length > 0, "required operation can not be null");
        Table table = tableFactory.openTable(tableName);
        Condition condition = table.newCondition();
        condition.EQ("operation", operation);
