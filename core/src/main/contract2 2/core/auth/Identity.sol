...
contract Identity {
    ...
    function syncExternal() public {
        aclManager.initExternal(address(this),externalAddress);
    }
    ...
    function removeIdFromRole(address idAddr, address roleAddr, bytes32[4] sign) public checkSign(sign) returns (bool){
        require(rolesManage.contains(roleAddr), "role is not exist!");
        Role role = Role(roleAddr);
    ...
    function addDetailToRes(address resourceGroup, address resource, string operation, bytes detail, bytes32[4] sign) public checkSign(sign) checkHolder(resource) returns (bool){
        require(resourceManage.contains(resourceGroup), "resource is not exist!");
        ResourceGroup res = ResourceGroup(resourceGroup);
    ...
    function removeOperationFromRes(address resourceGroup, address resource, string operation, bytes32[4] sign) public checkSign(sign) checkHolder(resource)   returns (bool){
        require(resourceManage.contains(resourceGroup), "resource is not exist!");
        ResourceGroup res = ResourceGroup(resourceGroup);
    ...
    function removeDetailTFromRes(address resourceGroup, address resource, string operation, bytes detail, bytes32[4] sign) public checkSign(sign) checkHolder(resource)  returns (bool){
        require(resourceManage.contains(resourceGroup), "resource is not exist!");
        ResourceGroup res = ResourceGroup(resourceGroup);
    ...
    function grant(address resource, address allowId, string operation, bytes detail, bytes32[4] sign) public checkSign(sign)checkHolder(resource)  returns (bool){
        return aclManager.grant(resource, allowId, operation, detail);
    }


    function revoke(address resource, address allowId, string operation, bytes detail, bytes32[4] sign) public checkSign(sign)  checkHolder(resource)  returns (bool){
