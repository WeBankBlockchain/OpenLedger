...
contract BaseResourceWithRole is BaseResource {
    ...
    function addOperationToResGroup(string roleName, string[] operations, bytes32[4] sign) public holderNotNull returns (bool) {
        require(roles[roleName] != address(0), "role has not  been create");
        return IdInterface(holder).addOperationToRes(resourceGroup[roleName], address(this), operations, sign);
    }

    function addDetailToResGroup(string roleName, string operation, bytes detail, bytes32[4] sign) public holderNotNull returns (bool) {
        require(roles[roleName] != address(0), "role has not  been create");
        return IdInterface(holder).addDetailToRes(resourceGroup[roleName], address(this), operation, detail, sign);
    }

    function removeOperation(string roleName, string operation, bytes32[4] sign) public holderNotNull returns (bool) {
        require(roles[roleName] != address(0), "role has not  been create");
        return IdInterface(holder).removeOperationFromRes(resourceGroup[roleName], address(this), operation, sign);
    }

    function removeDetail(string roleName, string operation, bytes detail, bytes32[4] sign) public holderNotNull returns (bool) {
        require(roles[roleName] != address(0), "role has not  been create");
        return IdInterface(holder).removeDetailTFromRes(resourceGroup[roleName], address(this), operation, detail, sign);
    }

    function checkAuth(bytes32[4] sign, string roleName, string functionName, bytes detail) internal holderNotNull returns (bool isCheck){
