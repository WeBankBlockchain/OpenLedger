...
contract NonFungibleBook {
    event InsertResult(address[] transactionAddress, uint256 noteId, bytes[] stringValueList, int256 count);
    ...
    function write(address[] transactionAddress, uint256 noteId, bytes[] memory stringValueList) public returns (bool, uint[2]){
    ...
    function setRecordEntry(uint termNo, uint seqNo, address[] transactionAddress, uint256 noteId, bytes[] stringValueList) internal returns (Entry){
        Table table = tableFactory.openTable(tableName);
